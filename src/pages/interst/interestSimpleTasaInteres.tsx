...
export function InterestSimpleTasaInteres() {
    ...
    const onSubmit = (data: FormData) => {
        ...
        const tasaInteres = (data.interestRate / (data.capital * tiempo))*100;
        setValorFuturo(tasaInteres);
    };

    return (
        <Col css={{ padding: '2rem' }}>
            <Text h1 size={30} color='#ffffff' css={{ letterSpacing: '1px', fontWeight: '$thin', marginTop: '2rem' }}>Calcular tasa de interes</Text>
            <Spacer y={1.2} />
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <Col>
                            <Col css={{ marginLeft: '2rem' }}>
                                ...
                                <Spacer y={0.7} />
                                <Input  {...register("capital")} clearable label="Capital a invertir" type='number' width='20.7rem' />
                                <Spacer y={0.7} />
                                <Row align='center'>
                                    <Input  {...register("interestRate")} clearable label="Interes producidos" type='number' width='10rem' />
                                    <Spacer x={0.6} />
                                    <Select >
                                        {options.map((option) => (
